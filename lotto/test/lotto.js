var p = [];
for (var i = 0; i < 6; i++) {
    var num = parseInt(prompt((i + 1) + "번째 번호를 입력하세요 (1~45):"), 10);
    // 잘못된 입력 방지 (중복/범위)
    while (isNaN(num) || num < 1 || num > 45 || p.includes(num)) {
        num = parseInt(prompt("잘못된 번호입니다. 다시 입력하세요 (1~45, 중복X):"), 10);
    }
    p.push(num);
}

document.write("내 로또 번호 : " + p);
